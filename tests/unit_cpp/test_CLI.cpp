...
TEST_F(CwdGuard, SimpleInitialization) {
  ...
  std::vector<const char*> argv = {"dummysolver",
                                   "--geometry",
                                   "=geom.vti", // Test for trailing equal until 4.0
                                   "--loadcase",
                                   "load.yaml",
                                   "--materialconfig",
                                   "material.yaml"};
...
TEST_F(CwdGuard, InitializationWorkingdirNumerics) {
  int mpi_world_rank = 0;
  TempDirGuard test_workdir("damask_cli_test_wd");
  ...
  EXPECT_EQ(fs::current_path(), test_workdir.path);
}

TEST_F(CwdGuard, InitializationInvalidWorkingDirectory) {
  int mpi_world_rank = 0;
  IOMockGuard io_mock;
  TempDirGuard tempdir("damask_cli_missing_wd");
...
TEST_F(CwdGuard, HelpExitsWithoutError) {
  int mpi_world_rank = 0;
  std::vector<const char*> argv = {"dummysolver", "-h"};
  auto args = std::span(argv.data(), std::size(argv));
  EXPECT_EXIT(
      {
        CLI cli(args, &mpi_world_rank);
...
TEST_F(CwdGuard, InitializationMissingLoadcaseValue) {
  ...
  std::vector<const char*> argv = {"dummysolver", "-g", "20grains16x16x16.vti", "-m", "material.yaml"};
  auto args = std::span(argv.data(), std::size(argv));
...
TEST_F(CwdGuard, InitializationMissingArgumentAfterFlag) {
  ...
  std::vector<const char*> argv = {"dummysolver", "-g", "20grains16x16x16.vti", "-m", "material.yaml", "-l"};
  auto args = std::span(argv.data(), std::size(argv));
