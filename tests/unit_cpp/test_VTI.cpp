...
namespace {
// stub: never called, but needed for linking
extern "C" void F_IO_error(int error_ID, const char* msg) {}

struct FIOErrorCalled final {};

std::string& last_f_io_error_msg() {
    static std::string msg;
    return msg;
}

void mock_io_error(int error_ID, const char* msg) {
    (void)error_ID;
    last_f_io_error_msg() = msg ? msg : "";
...
TEST(DecodeUncompressedVTI, Uncompressed32BitUnderflowError) {
    ...
    VTI vti(io);
    EXPECT_THROW((void)vti.decode_uncompressed_VTI(bad, n_bytes_per_word_32bit), FIOErrorCalled);
