...
then
 ...
 then
  ...
  RUN_JOB2="$MPI_ROOT/bin/mpirun ${MPIRUNOPTIONS} -cpu_bind=rr -prot -f "
  RUN_JOB1="$MPI_ROOT/bin/mpirun ${MPIRUNOPTIONS} -cpu_bind=rr -prot -w $MPIHPSPECIAL -np "
...
# always set VKISOLVER, CASISOLVER and MARC_DLL to NONE for MD Nastran
if test "$_OEM_NASTRAN" -ne 0
then
 VKISOLVER=NONE
 CASISOLVER=NONE
...
FORTRANMUMPS="$FCOMP -fpp -D_IMPLICITNONE $I8FFLAGS $I8DEFINES $DDM"
CCMUMPS=icc
...
#  Mumps solver compilation flags 
ORDERINGSF="-Dpord"
ORDERINGSC="-Dpord"
CDEFS="-DAdd_"
OPTF="-Dintel_ -DALLOW_NON_INIT -nofor_main"
OPTL="-nofor_main"
OPTC=" "
...
if test "$MUMPSSOLVER" = MUMPS
then
  MUMPSSOLVERLIBS="$MARC_LIB/libzmumps.a $MARC_LIB/libdmumps.a $MARC_LIB/libpord.a"
  if test $MPITYPE = none
  then
   echo hello > /dev/null
  fi
