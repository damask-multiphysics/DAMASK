...
class TestSeeds:
    ...
    def test_from_random(self,np_rng,cells):
    ...
    def test_from_Poisson_disc(self,np_rng,periodic):
        N_seeds = np_rng.integers(30,300)
    ...
    @pytest.mark.parametrize('periodic',[True,False])
    def test_from_Poisson_disc_invalid(self,np_rng,periodic):
        N_seeds = np_rng.integers(30,300)
        N_candidates = N_seeds//15
        distance = np_rng.random()
    ...
    def test_from_grid_reconstruct(self,np_rng):
    ...
    def test_from_grid_grid(self,np_rng,periodic,average):
        cells = np_rng.integers(10,20,3)
        size  = np.ones(3) + np_rng.random(3)
    ...
    def test_from_grid_selection(self,np_rng,periodic,average,invert):
        cells = np_rng.integers(10,20,3)
        N_seeds = np_rng.integers(30,300)
        size = np.ones(3) + np_rng.random(3)
        coords = seeds.from_random(size,N_seeds,cells,rng_seed=np_rng)
